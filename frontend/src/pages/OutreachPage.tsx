...
import {
  ApiRequestError, campaignAction, createCampaign, deleteTemplate, disconnectSender,
  ...
  return <div className="space-y-5">
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {[
        ...
        ['Sent', totals.sent, 'Delivered to Gmail API'], ['Failed', totals.failed, 'Needs review'],
    ...
    {tab === 'campaigns' && <div className="space-y-5">
      <section className="card-surface rounded-[14px] p-6">
        ...
        {initialLeadIds.length > 0 && <div className="mt-5 grid gap-4 lg:grid-cols-2">
          ...
          <label><span className="mb-1.5 block text-xs font-bold text-[#555966]">Sender</span><select value={senderId} onChange={e=>setSenderId(e.target.value)} className="focus-ring h-11 w-full rounded-[9px] border border-[#DDDDE4] bg-white px-3 text-sm"><option value="">Choose Gmail sender</option>{senders.map(s=><option key={s.id} value={s.id}>{s.email}</option>)}</select></label>
    ...
    {tab === 'senders' && <section className="card-surface rounded-[14px] p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.08em] text-[#7B61FF]">Email sender</p><h2 className="mt-1 font-display text-xl font-bold text-[#22242B]">Gmail accounts</h2><p className="mt-1 max-w-2xl text-sm text-[#777A87]">Connect through Google OAuth. Passwords are never stored. Sending uses the Gmail API and only starts after campaign approval.</p></div><button disabled={connectingGmail} onClick={handleConnectGmail} className="focus-ring rounded-[9px] bg-[#14151C] px-4 py-2.5 text-sm font-bold text-white"><Icon name="mail" className="mr-2 inline h-4 w-4" />{connectingGmail?'Opening Google…':'Connect Gmail'}</button></div><div className="mt-5 divide-y divide-[#ECECF1] rounded-[10px] border border-[#E5E5EA]">{senders.map(s=><div key={s.id} className="flex items-center justify-between gap-3 p-4"><div><div className="font-bold text-[#2D3038]">{s.email}</div><div className="mt-1 text-xs text-[#858894]">{s.display_name || 'Gmail'} · {s.status}</div>{s.last_error&&<div className="mt-1 text-xs text-[#9D3D36]">{s.last_error}</div>}</div><button onClick={async()=>{const token=await getToken();await disconnectSender(token,s.id);await load()}} className="rounded-[8px] border border-[#DDDDE4] px-3 py-2 text-xs font-bold text-[#555966]">Disconnect</button></div>)}{!senders.length&&<div className="p-6 text-center text-sm text-[#858894]">No Gmail sender connected yet.</div>}</div>
      <div className="mt-6 rounded-[10px] border border-[#E5E5EA] bg-[#FAFAFB] p-4">
