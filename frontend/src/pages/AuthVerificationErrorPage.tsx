...
export function AuthVerificationErrorPage({ message, onRetry, onSignOut }: Props) {
  return (
    <main className="grid min-h-screen place-items-center bg-[#F7F7FA] px-4 py-10">
      <section className="card-surface w-full max-w-[560px] rounded-[18px] p-6 sm:p-8">
        <BrandMark className="h-12 w-12" alt="" />
        <div className="mt-5 text-xs font-bold uppercase tracking-[0.11em] text-[#7B61FF]">Backend authentication</div>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <button onClick={onRetry} className="focus-ring button-pop h-11 rounded-[10px] bg-[#7B61FF] px-5 text-sm font-bold text-white hover:bg-[#6E53F0]">Try again</button>
